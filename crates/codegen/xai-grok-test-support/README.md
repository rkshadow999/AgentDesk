...
| `env` | `grok_binary()` (`GROK_BINARY` env → `CARGO_BIN_EXE` → local debug build of `xai-grok-pager`), `git_workdir()` (temp git repo, forces full libgit2 init), `test_env_cmd_tokio(cmd, mock_url, home)` (sandboxed HOME and GROK_HOME; Windows also redirects USERPROFILE, APPDATA, and LOCALAPPDATA; plus mock endpoints and telemetry kill-switches). |
