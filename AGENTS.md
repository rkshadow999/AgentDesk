...
> 当前公开预览版本：**0.1.0-alpha.11**（Windows 11 x64）  
> 更新时间：2026-07-21
...
| **Windows Setup.exe**（无需管理员，默认装到 `%LOCALAPPDATA%\AgentDesk`） | https://update.rkshadow.com/install/AgentDesk-0.1.0-alpha.11-win-x64-Setup.exe |
...
| 你手里的包 | 建议 |
| --- | --- |
| 含自托管公钥的 alpha.6 selfhost / alpha.7–alpha.10 | 设置中检查更新 → 升到 **alpha.11** |
...
## 2. 当前 alpha.11 已交付的能力（用户可感知）

| 能力 | 说明 |
| --- | --- |
| 中文优先桌面 UI + 英文切换 | Web 文案即时切换；部分原生字符串可能需重启 |
| **应用图标（RK 品牌）** | exe / 开始菜单 / Setup 向导 / 桌面快捷方式使用统一 `.ico` 与磁贴 PNG |
| **会话完成桌面通知** | 任务完成/失败时 Windows 通知；**多会话各弹一条**（按 sessionId 分 Tag）；点击可回到对应会话 |
...
pwsh -NoProfile -ExecutionPolicy Bypass -File .\scripts\agentdesk\Publish-AgentDeskSelfHostedUpdate.ps1 `
  ...
  -ReleaseDirectory ".\artifacts\release-alpha.11-selfhost\AgentDesk-0.1.0-alpha.11-win-x64"
```
